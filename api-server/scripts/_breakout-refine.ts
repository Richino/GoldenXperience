/**
 * Breakout pocket refinement — only family near flat/positive on holdout.
 * Filters selected on DEV only; holdout reported once per variant (no peeking loop).
 */
...
type Setup = {
  ...
  entry: number; stop: number; qi: number;
  spreadPips: number; rangeAtr: number; hourUtc: number; emaAligned: boolean; bodyAtr: number;
};
...
for (const instrument of PAIRS) {
  ...
  for (let i = 40; i < candles.length; i += 1) {
    ...
    setups.push({
      instrument, ms: atMs, direction, entry, stop, qi, spreadPips, rangeAtr,
...
console.log("breakout setups", setups.length);

type Filter = (s: Setup) => boolean;
const variants: Array<{ name: string; targetR: number; filter: Filter }> = [
  { name: "base 1R", targetR: 1, filter: () => true },
  { name: "spread≤1.2 1R", targetR: 1, filter: (s) => s.spreadPips <= 1.2 },
  { name: "spread≤1.0 1R", targetR: 1, filter: (s) => s.spreadPips <= 1.0 },
  { name: "range≤2.5ATR 1R", targetR: 1, filter: (s) => s.rangeAtr <= 2.5 },
  { name: "range≤2.0ATR 1R", targetR: 1, filter: (s) => s.rangeAtr <= 2.0 },
  { name: "EMA aligned 1R", targetR: 1, filter: (s) => s.emaAligned },
  { name: "body≥0.4ATR 1R", targetR: 1, filter: (s) => s.bodyAtr >= 0.4 },
  { name: "London 7-11UTC 1R", targetR: 1, filter: (s) => s.hourUtc >= 7 && s.hourUtc <= 11 },
  { name: "NY 12-16UTC 1R", targetR: 1, filter: (s) => s.hourUtc >= 12 && s.hourUtc <= 16 },
  { name: "tight+EMA+body 1R", targetR: 1, filter: (s) => s.spreadPips <= 1.2 && s.emaAligned && s.bodyAtr >= 0.35 },
  { name: "tight+range≤2.5+EMA 1R", targetR: 1, filter: (s) => s.spreadPips <= 1.2 && s.rangeAtr <= 2.5 && s.emaAligned },
  { name: "tight+London+EMA 1R", targetR: 1, filter: (s) => s.spreadPips <= 1.2 && s.hourUtc >= 7 && s.hourUtc <= 11 && s.emaAligned },
  { name: "tight+EMA+body 0.75R", targetR: 0.75, filter: (s) => s.spreadPips <= 1.2 && s.emaAligned && s.bodyAtr >= 0.35 },
  { name: "tight+EMA+body 1.25R", targetR: 1.25, filter: (s) => s.spreadPips <= 1.2 && s.emaAligned && s.bodyAtr >= 0.35 },
  { name: "EUR only tight+EMA 1R", targetR: 1, filter: (s) => s.instrument === "EUR_USD" && s.spreadPips <= 1.2 && s.emaAligned },
  { name: "JPY only tight+EMA 1R", targetR: 1, filter: (s) => s.instrument === "USD_JPY" && s.spreadPips <= 1.5 && s.emaAligned },
];
...
console.log("\n=== HOLDOUT (all variants with n≥40 on dev) ===");
for (const row of [...dev].sort((a, b) => b._mean - a._mean)) {
  ...
  console.log(`${row.name}: dev ${row.avg} (${row.verdict}, n=${row.n}) → hold ${hold.avg} (${hold.verdict}, n=${hold.n}) ${hold.ci}`);
}
process.exit(0);
