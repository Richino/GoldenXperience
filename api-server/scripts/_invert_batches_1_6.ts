/**
 * Actual-trade inversion for paper batches 1..6 (the batches recorded BEFORE
 * the four-family engine was fully live). RESEARCH ONLY — nothing in the
 * database, in paper positions, or in production is touched.
 *
 * Mirrors _actual_inversion.ts: uses only trades the paper engine actually
 * opened, resolves the counterfactual with the production `labelOutcome`
 * resolver (16:45 ET forced close, 48h horizon, same-candle ambiguity), fills
 * the inverse on the other side of the bid/ask book at the original entry bar,
 * mirrors stop and target around that fill, and pays its own spread.
 */
...
const OUT = process.env.OUT ?? path.join(serviceRoot, "..", "actual-inversion-batches-1-6.json");
...
console.log(`batch 1..6 trades found: ${trades.rows.length}`);
...
async function fetchAllBars(inst: string, fromIso: string, toIso: string): Promise<Q[]> {
  ...
  // OANDA caps count at 5000; loop until we pass toIso.
  for (let page = 0; page < 20; page++) {
    ...
    // next page starts one minute after the last close
...
writeFileSync(OUT, JSON.stringify({
  scope: { engine: "pre-four-family (paper batches 1..6)", found: trades.rows.length,
...
function push(a: Agg, r: number, pl: number | null) {
  a.n += 1;
  a.totalR += r; if (pl !== null && Number.isFinite(pl)) a.totalPl += pl;
  if (r > 0) { a.wins += 1; a.grossWinR += r; }
  else if (r < 0) { a.losses += 1; a.grossLossR += r; }
}
function fmt(a: Agg): string {
  ...
  return `n=${a.n} W/L=${a.wins}/${a.losses} winrate=${wr.toFixed(1)}% exp=${exp.toFixed(3)}R/trade totalR=${a.totalR >= 0 ? "+" : ""}${a.totalR.toFixed(2)} totalPl=$${a.totalPl.toFixed(2)} PF=${Number.isFinite(pf) ? pf.toFixed(2) : "∞"}`;
}
...
for (const r of out) {
  const origR = r.origR as number;
  const origPl = r.origPl as number | null;
...
console.log(`\n=== OVERALL (batches 1..6, matched sample where inv is resolvable) ===`);
