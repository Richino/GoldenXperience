// Frozen-cohort CADJPY Bull Break Extreme V1 executable replay. RESEARCH / PAPER ONLY.
// Replays the exact 84 authoritative TradingView trades against OANDA Practice M1 bid/ask.
// No strategy rule is altered; no signal cohort is regenerated; no deployment / broker orders.
import fs from 'node:fs';
import path from 'node:path';
import { config } from 'dotenv';

const SOURCE = 'C:/Users/arche/Downloads/GX_CADJPY_Bull_Break_Extreme_V1_-_1_to_2_RR_OANDA_CADJPY_2026-09-06_db1e9.csv';
const PINE = 'C:/Users/arche/Documents/cadjpy.txt';
const BASE = path.resolve('research-v2/cadjpy-bull-break-extreme-v1-spread-validation');
const DATA = path.join(BASE, 'data');
const INSTRUMENT = 'CAD_JPY', PIP = 100, H = 3_600_000, M = 60_000;
const N_EXPECTED = 84, ORIGIN_HOUR = 12, EMA_FAST = 20, EMA_SLOW = 50, ATR_LEN = 14, HOLD = 3, CONSENSUS_MIN = 3;
const BODY_ATR_MIN = 0.50, EXTREME_PCT = 0.25;

type C = { t:number; bo:number; bh:number; bl:number; bc:number; ao:number; ah:number; al:number; ac:number };
type T = { n:number; entryWall:string; exitWall:string; entry:number; exit:number; signal:string; exitLabel:string; duration:number; pnlJpy:number; entryMs:number; exitMs:number };
...
function tradesCsv(rows:any[]){
  const cols=['trade_number','direction','tv_entry_timestamp','resolved_utc_entry','tv_exit_timestamp','resolved_utc_exit','tv_entry_price','oanda_mid_entry','oanda_bid_entry','oanda_ask_entry','entry_spread_pips','ema20','ema50','ema20_3','close_3','vote_trend','vote_price','vote_slope','vote_momentum','consensus_score','previous_high','origin_open','origin_high','origin_low','origin_close','body','body_r','close_position_pct','break_distance','break_distance_r','atr14','initial_risk_pips','original_stop','original_target','tv_exit_price','oanda_mid_exit','oanda_bid_exit','oanda_ask_exit','exit_spread_pips','tv_result_r','exec_result_r','execution_drag_r','tv_exit_reason','exec_exit_reason','spread_changed_outcome','ambiguous_intraminute','matched','unmatched_reason'];
...
function main_render(raw:any){
  ...
  // full-cohort frozen-R TV benchmark (all authoritative trades with a reconstructed R; independent of OANDA M1 replay availability)
  const midFullR=raw.rows.filter((x:any)=>x.tv_result_r!==''&&x.tv_result_r!=null).map((x:any)=>x.tv_result_r);
  const mf=stats(midFullR); const mfDd=dd(midFullR);

  // currency-weighted TradingView PF (from CSV Net PnL JPY, all authoritative trades)
  const pnl=raw.cohortPnl as number[]; const gpC=sum(pnl.filter(x=>x>0)),glC=Math.abs(sum(pnl.filter(x=>x<=0)));
  const currencyPf=glC?gpC/glC:Infinity, currencyTotal=sum(pnl);
  ...
  // break-distance cohorts (diagnostic only): break_distance = close - prevHigh
  ...
  // body-strength cohorts (diagnostic only — 0.50 ATR rule unchanged)
  const body=[{lab:'0.50 to <0.75R',lo:0.50,hi:0.75},{lab:'0.75 to <1.00R',lo:0.75,hi:1.00},{lab:'>=1.00R',lo:1.00,hi:Infinity}]
    .map(b=>{ const z=r.filter((x:any)=>x.body_r>=b.lo&&x.body_r<b.hi).map((x:any)=>x.exec_result_r),s=stats(z); return{...b,n:s.trades,wr:s.wr,pf:s.pf,exp:s.exp}; });

  // close-extreme cohorts (diagnostic only, intentionally overlapping): top 25% / 15% / 10%
  const extreme=[{lab:'top 25%',min:75},{lab:'top 15%',min:85},{lab:'top 10%',min:90}]
    .map(b=>{ const z=r.filter((x:any)=>x.close_position_pct>=b.min).map((x:any)=>x.exec_result_r),s=stats(z); return{...b,n:s.trades,wr:s.wr,pf:s.pf,exp:s.exp}; });

  // consensus-score cohorts (diagnostic only — >= +3 rule unchanged)
  const consensusSplit=[3,4].map(c=>{ const z=r.filter((x:any)=>x.consensus_score===c).map((x:any)=>x.exec_result_r),s=stats(z); return{c,n:s.trades,wr:s.wr,pf:s.pf,exp:s.exp}; });
  ...
  raw.summary={midFull:{...mf,maxDd:mfDd},mid:ms,exec:es,execOptimistic:eo,classification:cls,verdict,byYear,exit:{tp,sl,time},
    currency:{pf:currencyPf,grossProfit:gpC,grossLoss:glC,total:currencyTotal},
    ...
    changes,changedImpact,breakDistance:bd,bodyStrength:body,closeExtreme:extreme,consensusSplit};
  ...
  fs.writeFileSync(path.join(BASE,'CONSENSUS_METADATA.csv'),
    ['consensus_score,n,exec_wr_pct,exec_pf,exec_expectancy_r',
     ...consensusSplit.map(x=>`${x.c},${x.n},${fmt(x.wr,2)},${fmt(x.pf,3)},${fmt(x.exp)}`)].join('\n'));
  fs.writeFileSync(path.join(BASE,'BODY_STRENGTH_METADATA.csv'),
    ['body_r_bucket,n,exec_wr_pct,exec_pf,exec_expectancy_r',
     ...body.map(b=>`${b.lab},${b.n},${fmt(b.wr,2)},${fmt(b.pf,3)},${fmt(b.exp)}`)].join('\n'));
  fs.writeFileSync(path.join(BASE,'CLOSE_EXTREME_METADATA.csv'),
    ['close_extreme_cohort,n,exec_wr_pct,exec_pf,exec_expectancy_r',
     ...extreme.map(b=>`${b.lab},${b.n},${fmt(b.wr,2)},${fmt(b.pf,3)},${fmt(b.exp)}`)].join('\n'));
  fs.writeFileSync(path.join(BASE,'BREAK_DISTANCE_METADATA.csv'),
    ['break_distance_r_bucket,n,exec_wr_pct,exec_pf,exec_expectancy_r',
     ...bd.map(b=>`${b.lab},${b.n},${fmt(b.wr,2)},${fmt(b.pf,3)},${fmt(b.exp)}`)].join('\n'));
  ...
  const report=[
    ...
    raw.matching.unmatched.length? ['','| # | TV timestamp | resolved UTC | reason |','|---|---|---|---|',...raw.matching.unmatched.map((u:any)=>`| ${u.trade} | ${u.tv} | ${u.utc} | ${u.reason} |`)].join('\n')
      : `All ${N_EXPECTED} authoritative trades matched executable OANDA data (no silent omissions).`,'',
    '## 2. Timezone / DST resolution','',
    `- Every entry resolves to **12:00 UTC**: **${raw.timezone.allAtOrigin}/${N_EXPECTED}**.`,
    ...
    `1. Exact LONG entries: **${N_EXPECTED}** — all \`CADJPY_1200_LONG\`.`,
    `2. Entries at 12:00 UTC: **${raw.timezone.allAtOrigin}/${N_EXPECTED}**.`,
    ...
    `4. EMA50 reconstructed on OANDA H1 midpoint (available on all ${p.ema50Available}/${N_EXPECTED}).`,
    `5. EMA20[3] and close[3] available on all ${p.ema50Available}/${N_EXPECTED} (3-bar lookback resolved).`,
    `6. Vote parity on OANDA mid — voteTrend (EMA20>EMA50): **${p.voteTrend}/${N_EXPECTED}**; votePrice (close>EMA20): **${p.votePrice}/${N_EXPECTED}**; voteSlope (EMA20>EMA20[3]): **${p.voteSlope}/${N_EXPECTED}**; voteMomentum (close>close[3]): **${p.voteMomentum}/${N_EXPECTED}**.`,
    `7. Consensus >= +${CONSENSUS_MIN} on OANDA mid: **${p.consensusOk}/${N_EXPECTED}** (mean consensus ${fmt(p.consensusMean,2)}).`,
    `8. close > high[1] (previous H1 high break): **${p.prevHighBreak}/${N_EXPECTED}**.`,
    `9. Bullish origin body (close > open): **${p.bullBodyOk}/${N_EXPECTED}**.`,
    `10. body >= 0.50·ATR14: **${p.strongBodyOk}/${N_EXPECTED}**.`,
    `11. close in upper 25% of candle range: **${p.upperExtremeOk}/${N_EXPECTED}**.`,
    `12. ATR14 parity: reconstructed ATR14 vs ATR implied by clean TP/SL exits — max diff **${fmt(p.maxAtrDiffPips,3)} pips** over ${p.atrCheckN} checkable trades.`,
    `13. Stop/target geometry: TP/SL move magnitude / ATR clusters at ~1R and ~2R (mean TP ${fmt(p.tpGeomMean,3)}R, mean SL ${fmt(p.slGeomMean,3)}R).`,
    `14. 3-H1 hold: max CSV duration **${p.maxDuration} bars**; all durations ≤ ${HOLD}: **${p.holdOk?'yes':'NO'}**.`,'',
    ...
    `- **Currency-weighted TradingView PF** (direct from CSV Net PnL JPY, all ${N_EXPECTED} trades): **${fmt(currencyPf,3)}** (gross profit ${fmt(gpC,3)} JPY / gross loss ${fmt(glC,3)} JPY; net ${fmt(currencyTotal,3)} JPY). Matches the exported ~1.762.`,
    `- **R-normalized TradingView/MID PF** (each trade normalized by its own frozen ATR14 R, all ${N_EXPECTED} trades): **${fmt(mf.pf,3)}**, expectancy **${fmt(mf.exp)}R/trade**, total **${fmt(mf.total)}R**. Matches the Phase-3B diagnostic ~1.729 / +0.319R / +26.81R.`,'',
    ...
    '## 5. Overall results','',
    `The frozen-R **TradingView benchmark** is reconstructed over all ${N_EXPECTED} authoritative trades. The **OANDA EXEC** column and the apples-to-apples **MID (matched)** column cover the ${es.trades} trades with complete executable M1 data (trade 31 excluded — 97-minute OANDA M1 gap during its hold window; it is a +${fmt(raw.rows.find((x:any)=>x.trade_number===31)?.tv_result_r??0,2)}R TV winner, so its removal lowers the matched-MID column below the full benchmark).`,'',
    '| Metric | TV benchmark (84) | MID (matched 83) | OANDA EXEC (83) |','|---|---:|---:|---:|',
    `| Trades | ${mf.trades} | ${ms.trades} | ${es.trades} |`,
    `| Wins | ${mf.wins} | ${ms.wins} | ${es.wins} |`,
    `| Losses | ${mf.losses} | ${ms.losses} | ${es.losses} |`,
    `| Win rate | ${fmt(mf.wr,2)}% | ${fmt(ms.wr,2)}% | ${fmt(es.wr,2)}% |`,
    `| Profit factor | ${fmt(mf.pf,3)} | ${fmt(ms.pf,3)} | ${fmt(es.pf,3)} |`,
    `| Total R | ${fmt(mf.total)}R | ${fmt(ms.total)}R | ${fmt(es.total)}R |`,
    `| Expectancy R/trade | ${fmt(mf.exp)}R | ${fmt(ms.exp)}R | ${fmt(es.exp)}R |`,
    `| Average winner R | ${fmt(mf.avgWin)}R | ${fmt(ms.avgWin)}R | ${fmt(es.avgWin)}R |`,
    `| Average loser R | ${fmt(mf.avgLoss)}R | ${fmt(ms.avgLoss)}R | ${fmt(es.avgLoss)}R |`,
    `| Max drawdown R | ${fmt(mfDd)}R | ${fmt(dd(mid))}R | ${fmt(dd(ex))}R |`,'',
    `Reconstructed R-normalized TV benchmark from the authoritative cohort is **${fmt(mf.exp)}R/trade** (PF ${fmt(mf.pf,3)}, total ${fmt(mf.total)}R) — matches the cited PF 1.729 / +0.319R/trade. The apples-to-apples cost comparison (matched 83) is MID **${fmt(ms.exp)}R** → EXEC **${fmt(es.exp)}R**, i.e. **${fmt(ms.exp-es.exp)}R/trade** of spread/execution drag.`,'',
    ...
    '## 7. Exit analysis','',
    `EXEC exit counts — TP: **${tp.count}**, SL: **${sl.count}**, TIME_EXIT: **${time.count}**. TradingView cohort: TP_OR_SL **59**, TIME_EXIT **25** (Phase-3B diagnostic decomposition ≈ TP 23 / SL 36 / TIME 25).`,'',
    ...
    ...body.map(b=>`| ${b.lab} | ${b.n} | ${fmt(b.wr,2)}% | ${fmt(b.pf,3)} | ${fmt(b.exp)}R |`),'',
    ...
    ...extreme.map(b=>`| ${b.lab} | ${b.n} | ${fmt(b.wr,2)}% | ${fmt(b.pf,3)} | ${fmt(b.exp)}R |`),'',
    '## 14. Consensus-score metadata (diagnostic only — >= +3 rule unchanged)','',
    '| Consensus | N | EXEC WR | EXEC PF | EXEC expectancy |','|---|---:|---:|---:|---:|',
    ...consensusSplit.map(x=>`| +${x.c} | ${x.n} | ${fmt(x.wr,2)}% | ${fmt(x.pf,3)} | ${fmt(x.exp)}R |`),'',
    '## Decision','',
    `1. **Does CADJPY V1 survive executable OANDA costs?** ${verdict==='SURVIVES_COSTS'?'Yes.':verdict==='MARGINAL_AFTER_COSTS'?'Marginally.':'No.'} (${verdict})`,
    ...
    `7. Average spread drag: **${fmt(avg(drag))}R/trade** (total ${fmt(sum(drag))}R).`,
    `8. Year stability: ${positive}/${withTrades} positive year-buckets — ${yearVerdict}`,
    `9. Do time exits materially hurt? ${timeVerdict}.`,
    `10. Does the pre-cost ~+0.319R Phase-3B edge survive executable pricing? Full-84 frozen-R benchmark is ${fmt(mf.exp)}R; on the matched 83 the MID edge is ${fmt(ms.exp)}R and after spread it is ${fmt(es.exp)}R — ${es.exp>=.10?'the edge largely survives.':es.exp>=.05?'roughly half the edge survives.':es.exp>=0?'almost none of the edge survives.':'the edge is erased and turns negative.'}`,
    `11. Should this exact V1 become the frozen CADJPY research/paper candidate? **${es.exp>=.10?'Yes — it clears the ≥+0.10R executable bar (subject to the small-sample and single-instrument caveats).':'No — executable expectancy does not clear the +0.10R candidate bar.'}**`,'',
    '---','',
  ...
  return{es,ms,verdict,cls,currencyPf};
}
...
async function main(){
  ...
  for(const t of ts) if(t.signal!=='CADJPY_1200_LONG'||new Date(t.entryMs).getUTCHours()!==ORIGIN_HOUR) throw Error(`trade ${t.n} fails frozen signal/time: ${t.signal} @ ${new Date(t.entryMs).toISOString()}`);
  ...
  const e20=ema(mid.map(x=>x.c),EMA_FAST);
  const e50=ema(mid.map(x=>x.c),EMA_SLOW);
  ...
  let voteTrendOk=0, votePriceOk=0, voteSlopeOk=0, voteMomentumOk=0, consensusOk=0, prevHighBreak=0, ema50Available=0;
  let bullBodyOk=0, strongBodyOk=0, upperExtremeOk=0; const consensusVals:number[]=[];
  ...
  for(const t of ts){
    ...
    if(i==null||i<3||atr[i]==null||e20[i]==null||e20[i-3]==null||e50[i]==null){ rows.push({...base,matched:false,unmatched_reason:i==null?'NO_H1_SIGNAL_BAR':'INDICATOR_WARMUP_UNAVAILABLE'}); continue; }

    const c=h1[i], mc=mid[i], em=mc.c, risk=atr[i]!;
    const ema20=e20[i]!, ema50=e50[i]!, ema20_3=e20[i-3]!, close3=mid[i-3].c;
    // four-vote consensus on OANDA mid
    const vTrend=ema20>ema50?1:ema20<ema50?-1:0;
    const vPrice=em>ema20?1:em<ema20?-1:0;
    const vSlope=ema20>ema20_3?1:ema20<ema20_3?-1:0;
    const vMom=em>close3?1:em<close3?-1:0;
    const consensus=vTrend+vPrice+vSlope+vMom;
    ema50Available++;
    // origin candle (OANDA mid) body + extreme
    const oOpen=mc.o,oHigh=mc.h,oLow=mc.l,oClose=mc.c;
    const bodyAbs=Math.abs(oClose-oOpen), candleRange=oHigh-oLow;
    const bodyR=bodyAbs/risk;
    const closePosPct=candleRange>0?(oClose-oLow)/candleRange*100:0;
    const bullBody=oClose>oOpen, strongBody=bodyAbs>=BODY_ATR_MIN*risk, upperExtreme=candleRange>0&&oClose>=oHigh-candleRange*EXTREME_PCT;
    // previous H1 high (single bar) from mid
    const j1=ix.get(t.entryMs-H);
    const prevHigh=j1!=null?mid[j1].h:NaN;
    const breakDist=Number.isNaN(prevHigh)?NaN:em-prevHigh, breakDistR=Number.isNaN(breakDist)?NaN:breakDist/risk;
    const stop=em-risk*1, target=em+risk*2;

    // parity tallies (no filtering)
    if(vTrend===1)voteTrendOk++;
    if(vPrice===1)votePriceOk++;
    if(vSlope===1)voteSlopeOk++;
    if(vMom===1)voteMomentumOk++;
    consensusVals.push(consensus);
    if(consensus>=CONSENSUS_MIN)consensusOk++;
    if(!Number.isNaN(prevHigh)&&em>prevHigh)prevHighBreak++;
    if(bullBody)bullBodyOk++;
    if(strongBody)strongBodyOk++;
    if(upperExtreme)upperExtremeOk++;
    ...
    // executable entry at ASK close of 12:00 bar
    const entryAsk=c.ac;
    const missing:number[]=[]; for(let at=t.entryMs+H;at<t.entryMs+4*H;at+=M) if(!mt.has(at))missing.push(at);
    const common:any={...base,oanda_mid_entry:em,oanda_bid_entry:c.bc,oanda_ask_entry:c.ac,entry_spread_pips:(c.ac-c.bc)*PIP,ema20,ema50,ema20_3,close_3:close3,vote_trend:vTrend,vote_price:vPrice,vote_slope:vSlope,vote_momentum:vMom,consensus_score:consensus,previous_high:Number.isNaN(prevHigh)?'':prevHigh,origin_open:oOpen,origin_high:oHigh,origin_low:oLow,origin_close:oClose,body:bodyAbs,body_r:bodyR,close_position_pct:closePosPct,break_distance:Number.isNaN(breakDist)?'':breakDist,break_distance_r:Number.isNaN(breakDistR)?'':breakDistR,atr14:risk,initial_risk_pips:risk*PIP,original_stop:stop,original_target:target};
  ...
  // discrepancy notes
  if(consensusOk!==N_EXPECTED)discrepancies.push(`consensus>=+${CONSENSUS_MIN} holds on ${consensusOk}/${N_EXPECTED} on OANDA mid (mid-feed reconstruction; TV feed may differ marginally near vote boundaries).`);
  ...
  if(bullBodyOk!==N_EXPECTED)discrepancies.push(`bullish body (close>open) holds on ${bullBodyOk}/${N_EXPECTED} on OANDA mid.`);
  if(strongBodyOk!==N_EXPECTED)discrepancies.push(`body>=0.50·ATR14 holds on ${strongBodyOk}/${N_EXPECTED} on OANDA mid (ATR reconstructed from mid).`);
  if(upperExtremeOk!==N_EXPECTED)discrepancies.push(`close in upper 25% holds on ${upperExtremeOk}/${N_EXPECTED} on OANDA mid (feed rounding near boundary).`);
  ...
  const samples=[seg('begin',()=>true),{seg:'middle',trade:ts[Math.floor(N_EXPECTED/2)].n,tv:ts[Math.floor(N_EXPECTED/2)].entryWall,utc:new Date(ts[Math.floor(N_EXPECTED/2)].entryMs).toISOString()},{seg:'end',trade:ts.at(-1)!.n,tv:ts.at(-1)!.entryWall,utc:new Date(ts.at(-1)!.entryMs).toISOString()},seg('EST(winter,07:00NY)',x=>x.entryWall.includes(' 07:00')),seg('EDT(summer,08:00NY)',x=>x.entryWall.includes(' 08:00')),seg('pre-Nov-DST(EDT,08:00NY)',x=>x.entryWall.startsWith('2023-10-12')),seg('post-Nov-DST(EST,07:00NY)',x=>x.entryWall.startsWith('2023-12-11'))].filter(Boolean);
  ...
  const raw:any={ generatedAt:new Date().toISOString(), instrument:INSTRUMENT,
    ...
    authoritativeCohort:{ file:path.basename(SOURCE), trades:N_EXPECTED, wins:46, losses:38, winRatePct:54.76, headlineProfitFactorRnorm:1.729, expectancyRnorm:0.319, currencyProfitFactor:1.762, signal:'CADJPY_1200_LONG', tpOrSl:59, timeExit:25, phase3bDecomp:{tp:23,sl:36,time:25}, entryRange:['2023-01-05','2026-08-31'] },
    cohortPnl:ts.map(t=>t.pnlJpy),
    ...
    parity:{ maxEntryDiffPips:Math.max(...entryDiffs), medianEntryDiffPips:median(entryDiffs), voteTrend:voteTrendOk, votePrice:votePriceOk, voteSlope:voteSlopeOk, voteMomentum:voteMomentumOk, consensusOk, consensusMean:consensusVals.length?consensusVals.reduce((s,x)=>s+x,0)/consensusVals.length:0, prevHighBreak, bullBodyOk, strongBodyOk, upperExtremeOk, ema50Available, maxAtrDiffPips:atrDiffs.length?Math.max(...atrDiffs):0, atrCheckN, tpGeomMean:tpGeom.length?tpGeom.reduce((s,x)=>s+x,0)/tpGeom.length:0, slGeomMean:slGeom.length?slGeom.reduce((s,x)=>s+x,0)/slGeom.length:0, maxDuration:Math.max(...ts.map(x=>x.duration)), holdOk:ts.every(x=>x.duration<=HOLD), discrepancies },
    ambiguous, rows };
  const {es,verdict,cls,currencyPf}=main_render(raw);
  console.log(`CADJPY V1 replay complete: ${rows.length} rows, ${matched.length} matched, ${ambiguous.length} ambiguous.`);
  console.log(`Currency-weighted TV PF ${currencyPf.toFixed(3)} | R-norm MID PF ${raw.summary.mid.pf.toFixed(3)} exp ${raw.summary.mid.exp.toFixed(4)}R`);
