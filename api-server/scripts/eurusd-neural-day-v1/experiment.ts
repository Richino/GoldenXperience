...
type Direction = 1 | -1;
type Outcome = {
  kind: "TARGET" | "STOP" | "AMBIGUOUS_STOP" | "TIME_EXIT";
  r: number;
  exitTime: number;
  holdMinutes: number;
};
...
function nearestNewsMinutes(newsTimes: number[], time: number) {
  let low = 0;
...
function regimeVector(bars: Bar[], series: Series, index: number) {
...
function fitRegimes(vectors: number[][], count = 3) {
...
function regimeOf(vector: number[], centroids: number[][]) {
  let best = 0;
...
function rawFeatures(bars: Bar[], series: Series, index: number, regime: number) {
...
function resolveOutcome(bars: Bar[], series: Series, index: number, direction: Direction): Outcome {
...
function buildCandidates(bars: Bar[], series: Series, centroids: number[][], newsTimes: number[]) {
...
function rowsIn(candidates: Candidate[], period: { from: number; to: number }) {
...
function marketDays(candidates: Candidate[]) {
...
function summarize(trades: Trade[], days: number): Summary {
