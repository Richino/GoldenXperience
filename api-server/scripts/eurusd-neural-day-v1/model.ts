...
export function trainNeuralModel(
  samples: Sample[],
  architecture: Architecture,
  options: { seed: number; epochs: number; learningRate: number; l2: number },
): NeuralModel {
  ...
  const positiveWeight = samples.length / Math.max(1, 2 * positives);
  const negativeWeight = samples.length / Math.max(1, 2 * (samples.length - positives));
