// Frozen-cohort GBPJPY 11UTC 3-Bar Break V1 executable replay. RESEARCH / PAPER ONLY.
// Replays the exact 79 authoritative TradingView trades against OANDA Practice M1 bid/ask.
// No strategy rule is altered; no signal cohort is regenerated; no deployment / broker orders.
import fs from 'node:fs';
import path from 'node:path';
import { config } from 'dotenv';

const SOURCE = 'C:/Users/arche/Downloads/GX_GBPJPY_11UTC_3-Bar_Break_V1_-_1_to_2_RR_OANDA_GBPJPY_2026-09-06_6781b.csv';
const PINE = 'C:/Users/arche/Documents/gbpjpy.txt';
const BASE = path.resolve('research-v2/gbpjpy-11utc-3bar-break-v1-spread-validation');
const DATA = path.join(BASE, 'data');
const INSTRUMENT = 'GBP_JPY', PIP = 100, H = 3_600_000, M = 60_000;
const N_EXPECTED = 79, ORIGIN_HOUR = 11, EMA_FAST = 20, EMA_SLOW = 50, ATR_LEN = 14, HOLD = 3, CONSENSUS_MIN = 3;
...
function main_render(raw:any){
  ...
  // break-distance cohorts (diagnostic only): break_distance = close - prev3High
  ...
  raw.summary={mid:ms,exec:es,execOptimistic:eo,classification:cls,verdict,byYear,exit:{tp,sl,time},
    ...
    changes,changedImpact,breakDistance:bd,consensusSplit};
  ...
  fs.writeFileSync(path.join(BASE,'BREAKOUT_STRENGTH_METADATA.csv'),
  ...
  const report=[
    ...
    `- Every entry resolves to **11:00 UTC**: **${raw.timezone.allAtOrigin}/${N_EXPECTED}**.`,
    ...
    `1. Exact LONG entries: **${N_EXPECTED}** — all \`GBPJPY_1100_LONG\`.`,
    `2. Entries at 11:00 UTC: **${raw.timezone.allAtOrigin}/${N_EXPECTED}**.`,
    ...
    `5. Vote parity on OANDA mid — voteTrend (EMA20>EMA50): **${p.voteTrend}/${N_EXPECTED}**; votePrice (close>EMA20): **${p.votePrice}/${N_EXPECTED}**; voteSlope (EMA20>EMA20[3]): **${p.voteSlope}/${N_EXPECTED}**; voteMomentum (close>close[3]): **${p.voteMomentum}/${N_EXPECTED}**.`,
    `6. Consensus >= +${CONSENSUS_MIN} on OANDA mid: **${p.consensusOk}/${N_EXPECTED}** (mean consensus ${fmt(p.consensusMean,2)}).`,
    `7. close > high[1]: **${p.prevHighBreak}/${N_EXPECTED}**.`,
    `8. close > max(high[1],high[2],high[3]) (prev3High): **${p.threeBarBreak}/${N_EXPECTED}**.`,
    `9. ATR14 parity: reconstructed ATR14 vs ATR implied by clean TP/SL exits — max diff **${fmt(p.maxAtrDiffPips,3)} pips** over ${p.atrCheckN} checkable trades.`,
    `10. Stop/target geometry: TP/SL move magnitude / ATR clusters at ~1R and ~2R (mean TP ${fmt(p.tpGeomMean,3)}R, mean SL ${fmt(p.slGeomMean,3)}R).`,
    `11. 3-H1 hold: max CSV duration **${p.maxDuration} bars**; all durations ≤ ${HOLD}: **${p.holdOk?'yes':'NO'}**.`,'',
    ...
    `- **Currency-weighted TradingView PF** (direct from CSV Net PnL JPY): **${fmt(currencyPf,3)}** (gross profit ${fmt(gpC,3)} JPY / gross loss ${fmt(glC,3)} JPY; net ${fmt(currencyTotal,3)} JPY). Matches the exported ~1.68.`,
    `- **R-normalized TradingView/MID PF** (each trade normalized by its own frozen ATR14 R): **${fmt(ms.pf,3)}**, expectancy **${fmt(ms.exp)}R/trade**. Matches the Phase-4 diagnostic ~1.837 / +0.315R.`,'',
    ...
    `Reconstructed R-normalized MID/TV expectancy from the authoritative cohort is **${fmt(ms.exp)}R/trade** (MID PF ${fmt(ms.pf,3)}). The Pine header cites PF 1.837 / +0.315R/trade.`,'',
    ...
    '## 7. Exit analysis','',
    `EXEC exit counts — TP: **${tp.count}**, SL: **${sl.count}**, TIME_EXIT: **${time.count}**. TradingView cohort: TP_OR_SL **46**, TIME_EXIT **33** (Phase-4 diagnostic decomposition ≈ TP 18 / SL 28 / TIME 33).`,'',
    ...
    '## Decision','',
    `1. **Does GBPJPY V1 survive executable OANDA costs?** ${verdict==='SURVIVES_COSTS'?'Yes.':verdict==='MARGINAL_AFTER_COSTS'?'Marginally.':'No.'} (${verdict})`,
    ...
    `10. Does the pre-cost ~+0.315R Phase-4 edge survive executable pricing? Reconstructed MID edge is ${fmt(ms.exp)}R; after spread it is ${fmt(es.exp)}R — ${es.exp>=.10?'the edge largely survives.':es.exp>=.05?'roughly half the edge survives.':es.exp>=0?'almost none of the edge survives.':'the edge is erased and turns negative.'}`,
    `11. Should this exact V1 become the frozen GBPJPY research/paper candidate? **${es.exp>=.10?'Yes — it clears the ≥+0.10R executable bar (subject to the small-sample and single-instrument caveats).':'No — executable expectancy does not clear the +0.10R candidate bar.'}**`,'',
    '---','',
...
async function main(){
  ...
  for(const t of ts) if(t.signal!=='GBPJPY_1100_LONG'||new Date(t.entryMs).getUTCHours()!==ORIGIN_HOUR) throw Error(`trade ${t.n} fails frozen signal/time: ${t.signal} @ ${new Date(t.entryMs).toISOString()}`);
  ...
  let voteTrendOk=0, votePriceOk=0, voteSlopeOk=0, voteMomentumOk=0, consensusOk=0, prevHighBreak=0, threeBarBreak=0, ema50Available=0; const consensusVals:number[]=[];
  ...
  for(const t of ts){
    ...
    if(i==null||atr[i]==null||e20[i]==null||e20[i-3]==null||e50[i]==null||i<3){ rows.push({...base,matched:false,unmatched_reason:i==null?'NO_H1_SIGNAL_BAR':'INDICATOR_WARMUP_UNAVAILABLE'}); continue; }
    ...
    ema50Available++;
    // prev highs from mid
    const j1=ix.get(t.entryMs-H),j2=ix.get(t.entryMs-2*H),j3=ix.get(t.entryMs-3*H);
    const prevHigh=j1!=null?mid[j1].h:NaN;
    const prev3High=(j1!=null&&j2!=null&&j3!=null)?Math.max(mid[j1].h,mid[j2].h,mid[j3].h):NaN;
    const breakDist=Number.isNaN(prev3High)?NaN:em-prev3High, breakDistR=Number.isNaN(breakDist)?NaN:breakDist/risk;
    ...
    if(!Number.isNaN(prev3High)&&em>prev3High)threeBarBreak++;
  ...
  if(threeBarBreak!==N_EXPECTED)discrepancies.push(`close>prev3High holds on ${threeBarBreak}/${N_EXPECTED} on OANDA mid (feed rounding near equality).`);
  ...
  const samples=[seg('begin',()=>true),{seg:'middle',trade:ts[Math.floor(N_EXPECTED/2)].n,tv:ts[Math.floor(N_EXPECTED/2)].entryWall,utc:new Date(ts[Math.floor(N_EXPECTED/2)].entryMs).toISOString()},{seg:'end',trade:ts.at(-1)!.n,tv:ts.at(-1)!.entryWall,utc:new Date(ts.at(-1)!.entryMs).toISOString()},seg('EST(winter,06:00NY)',x=>x.entryWall.includes(' 06:00')),seg('EDT(summer,07:00NY)',x=>x.entryWall.includes(' 07:00')),seg('post-Mar-DST',x=>x.entryWall.startsWith('2023-03-29')),seg('pre-Nov-DST',x=>x.entryWall.startsWith('2023-11-06'))].filter(Boolean);
  ...
  const raw:any={ generatedAt:new Date().toISOString(), instrument:INSTRUMENT,
    ...
    authoritativeCohort:{ file:path.basename(SOURCE), trades:N_EXPECTED, wins:44, losses:35, winRatePct:55.70, headlineProfitFactorRnorm:1.837, expectancyRnorm:0.315, signal:'GBPJPY_1100_LONG', tpOrSl:46, timeExit:33, phase4Decomp:{tp:18,sl:28,time:33}, entryRange:['2023-01-19','2026-08-20'] },
    ...
    parity:{ maxEntryDiffPips:Math.max(...entryDiffs), medianEntryDiffPips:median(entryDiffs), voteTrend:voteTrendOk, votePrice:votePriceOk, voteSlope:voteSlopeOk, voteMomentum:voteMomentumOk, consensusOk, consensusMean:consensusVals.length?consensusVals.reduce((s,x)=>s+x,0)/consensusVals.length:0, prevHighBreak, threeBarBreak, ema50Available, maxAtrDiffPips:atrDiffs.length?Math.max(...atrDiffs):0, atrCheckN, tpGeomMean:tpGeom.length?tpGeom.reduce((s,x)=>s+x,0)/tpGeom.length:0, slGeomMean:slGeom.length?slGeom.reduce((s,x)=>s+x,0)/slGeom.length:0, maxDuration:Math.max(...ts.map(x=>x.duration)), holdOk:ts.every(x=>x.duration<=HOLD), discrepancies },
    ambiguous, rows };
  const {es,verdict,cls,currencyPf}=main_render(raw);
  console.log(`GBPJPY V1 replay complete: ${rows.length} rows, ${matched.length} matched, ${ambiguous.length} ambiguous.`);
