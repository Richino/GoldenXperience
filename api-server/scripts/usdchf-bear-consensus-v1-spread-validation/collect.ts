// Collect OANDA practice bid/ask history for the USDCHF Bear Consensus V1 cohort.
...
import { parseCohort, INSTRUMENT, ORIGIN_HOUR_UTC, AUTHORITATIVE_TRADE_COUNT, type Candle } from './lib.js';
...
const DIR = path.resolve('research-v2/usdchf-bear-consensus-v1-spread-validation/data');
...
// Sanity: every leg is the exact SHORT signal, resolves to 11:00 UTC.
for (const t of trades) {
  if (t.tvSignal !== 'USDCHF_1100_SHORT') throw new Error(`Trade ${t.tradeNumber}: signal ${t.tvSignal}`);
...
console.log('All 103 legs are USDCHF_1100_SHORT and resolve to 11:00 UTC.');
...
// (entry+1h = 12:00 UTC) through a generous horizon past the 3rd future bar
// close (entry+4h = 15:00 UTC), so the executable side can resolve TP/SL. ---
