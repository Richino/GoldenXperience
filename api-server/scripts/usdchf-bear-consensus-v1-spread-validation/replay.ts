// Executable bid/ask replay of the frozen USDCHF Bear Consensus Structure V1 cohort.
// No optimization, no rule changes. Barrier levels are mid-referenced off the
// 11:00-open signal-bar close and frozen ATR14 (exactly as the Pine computes);
// only the EXECUTABLE side decides whether each level was actually reached.
//   SHORT -> filled at BID; target/stop/time-exit judged on ASK.
...
// --- consensus + structure recomputation (parity), exactly per Pine ---
function consensusAt(i: number): { score: number; votes: number[]; structure: boolean; ready: boolean } {
  ...
  const ready = e20 != null && e50 != null && e20b3 != null && atr != null && atr > 0 && c3 != null && hiPrev != null && loPrev != null;
  if (!ready) return { score: 0, votes: [0, 0, 0, 0], structure: false, ready: false };
  ...
  return { score, votes: [vTrend, vPrice, vSlope, vMom], structure, ready };
}
...
function classifyTv(t: TvTrade, atr: number): { r: number; reason: string } {
  const r = (t.tvEntryPrice - t.tvExitPrice) / atr; // short: entry - exit
  let reason: string;
...
for (const t of trades) {
  ...
  if (i == null || atrSeries[i] == null) {
    rows.push({
      ...
      consensus_score: NaN as any, consensus_votes: '', structure_ok: false, parity_consensus_ok: false, parity_structure_ok: false,
  ...
  const stop = entryMid + atr;          // 1 ATR above entry
  const target = entryMid - 2 * atr;    // 2 ATR below entry

  const con = consensusAt(i);
  const parityConsensus = con.ready && con.score <= -3;
  const parityStructure = con.structure;

  const tv = classifyTv(t, atr);

  // --- executable minute-by-minute replay (SHORT: exits judged on ASK) ---
  const replayStart = t.resolvedEntryUtcMs + HOUR;          // open of future #1 (12:00 UTC)
  const scanEnd = t.resolvedEntryUtcMs + (MAX_HOLD + 1) * HOUR; // close of future #3 (15:00 UTC)
  ...
  for (; idx < m1.length && m1[idx].t < scanEnd; idx++) {
    const c = m1[idx];
    minutesSeen++;
    const h = c.ah; // ask high (adverse for short = stop side)
    const l = c.al; // ask low  (favorable for short = target side)
    const o = c.ao;
  ...
  // TIME_EXIT: close of future #3 (the 14:00-open H1 bar) on the ASK.
  if (exitPrice == null) {
    const timeBarIdx = h1Idx.get(t.resolvedEntryUtcMs + MAX_HOLD * HOUR); // 14:00-open bar
    if (timeBarIdx == null) {
      rows.push({
        ...(base as Row),
        ...
        oanda_mid_exit: '', oanda_bid_exit: '', oanda_ask_exit: '', exit_spread_pips: '',
        consensus_score: con.score, consensus_votes: con.votes.join('|'), structure_ok: con.structure,
        parity_consensus_ok: parityConsensus, parity_structure_ok: parityStructure,
    ...
    exitPrice = tb.ac; // buy-to-close on ASK at future #3 close
    exitReason = 'TIME_EXIT';
    exitCandle = tb;
  }

  const execR = (entryBid - exitPrice) / atr; // short: sold at bid, bought back at exit
...
const cols = [
  ...
  'atr_at_entry', 'initial_risk_pips', 'original_stop', 'original_target',
  'tv_exit_price', 'oanda_mid_exit', 'oanda_bid_exit', 'oanda_ask_exit', 'exit_spread_pips',
  'consensus_score', 'consensus_votes', 'structure_ok', 'parity_consensus_ok', 'parity_structure_ok',
...
console.log(`Parity consensus<=-3 failures: ${parityConsensusFails.length}; structure LH+LL failures: ${parityStructureFails.length}.`);
