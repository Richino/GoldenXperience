...
export type NzdusdPriceBar = { open: number; high: number; low: number; close: number };
export type NzdusdHistoryCandle = Candle & { mid: NzdusdPriceBar; bid: NzdusdPriceBar; ask: NzdusdPriceBar };
export type NzdusdHistory = {
  symbol: "NZD_USD";
  timeframe: "H1";
  source: string;
  from: string;
  to: string;
  candles: NzdusdHistoryCandle[];
};

function asHistoryCandle(bar: ResearchCandle): NzdusdHistoryCandle {
...
export async function loadNzdusdHistory(fixturePath?: string): Promise<NzdusdHistory> {
  if (fixturePath) {
    const parsed = JSON.parse(await readFile(resolve(fixturePath), "utf8")) as Omit<NzdusdHistory, "source" | "from" | "to">;
    const candles = [...parsed.candles].sort((left, right) => Date.parse(left.time) - Date.parse(right.time));
    return { ...parsed, source: resolve(fixturePath), from: candles[0]?.time ?? "", to: candles.at(-1)?.time ?? "", candles };
  }

  loadEnvConfig(resolve(process.cwd(), "../api-server"));
  const { getResearchCandles } = await import("../src/lib/oanda/client");
  const from = process.env.NZDUSD_PARITY_FROM ?? "2023-01-01T00:00:00.000Z";
  const configuredTo = process.env.NZDUSD_PARITY_TO;
  let cursor = configuredTo;
  let priorEarliest = "";
  const byTime = new Map<string, NzdusdHistoryCandle>();

  for (let page = 0; page < 12; page += 1) {
    const batch = await getResearchCandles("NZD_USD", "H1", 5_000, cursor ? { to: cursor } : {});
  ...
  return {
    symbol: "NZD_USD", timeframe: "H1", source: "OANDA practice API H1 MBA candles", from,
