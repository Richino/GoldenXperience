/**
 * EURJPY 01-05 Range Break V1 Pine-parity historical replay.
...
const ORIGIN_HOUR = 6;
const CACHE_PATH = "../research/frozen-strategies/EURJPY/oanda-h1-mid.csv";
const TV_CSV_PATH = "../research/frozen-strategies/EURJPY/tradingview-trades.csv";
...
async function fetchMidpointH1(token: string): Promise<Candle[]> {
  ...
  let cursor = FETCH_START; const endMs = Date.parse(FETCH_END);
  for (let page = 0; page < 16; page += 1) {
    ...
    const raw = payload.candles ?? []; if (!raw.length) break;
...
function dataQuality(candles: readonly Candle[]) {
  ...
  return { first: candles[0]?.time ?? null, last: candles.at(-1)?.time ?? null, total: candles.length, incomplete, duplicates, outOfOrder, unexpectedGaps: gaps.length, unexpectedGapSample: gaps.slice(0, 12) };
}
...
function parseTradingViewEntries(): TvEntry[] { const rows = readFileSync(TV_CSV_PATH, "utf8").replace(/^﻿/, "").trim().split(/\r?\n/); const h = rows[0]!.split(","); const tC = h.indexOf("Type"), dC = h.indexOf("Date and time"); const out: TvEntry[] = []; for (const row of rows.slice(1)) { const cols = row.split(","); if ((cols[tC] ?? "").trim().toLowerCase() !== "entry long") continue; const wall = (cols[dC] ?? "").trim(); out.push({ wall, originUtc: nyWallToUtcISO(wall) }); } return out; }
...
async function main() {
  ...
  report.tradingViewEntries = tv.length; report.entriesNormalizedTo0600Utc = tvAt.length; report.entriesNotAt0600Utc = tvNot.length; report.tvNotSample = tvNot.slice(0, 10);
  if (tvNot.length > 0) { console.log(JSON.stringify({ ...report, halted: "A TradingView entry did not normalize to 06:00 UTC." }, null, 2)); process.exitCode = 1; return; }
  if (!token) { report.OANDA_H1_DATA_FETCHED = "NO"; report.verdict = "EURJPY_PARITY_BLOCKED_NO_MARKET_DATA"; console.log(JSON.stringify(report, null, 2)); process.exitCode = 1; return; }

  const candles = await fetchMidpointH1(token);
  if (!candles.length) { report.OANDA_H1_DATA_FETCHED = "NO"; report.verdict = "EURJPY_PARITY_BLOCKED_NO_MARKET_DATA"; console.log(JSON.stringify(report, null, 2)); process.exitCode = 1; return; }
  writeCache(candles);
  ...
  for (let i = 0; i < candles.length; i += 1) {
    ...
    const ev = evaluateEurjpy01To05RangeBreakV1(candles.slice(0, i + 1));
    if (ev.strategySignalQualified) signals.push({ ts: candles[i]!.time, ema20: ev.ema20, ema20Back: ev.ema20Back, atr14: ev.atr14, preHigh: ev.preHigh, preLow: ev.preLow, previousHigh: ev.previousHigh, preRangeBarCount: ev.preRangeBarCount, emaSlopeUp: ev.emaSlopeUp, previousHighBreak: ev.previousHighBreak, preRangeHighBreak: ev.preRangeHighBreak, signalMidClose: ev.signalMidClose });
  }
  ...
  report.typescriptSignalsInWindow = tsInWindow.length; report.typescriptSignalsAllFrom2023 = signals.length; report.typescriptSignalsAfterLastTvEntry = tsAfter;
  report.exactMatches = exactMatches.length; report.tvOnly = tvOnly; report.tsOnly = tsOnly; report.directionMismatches = 0;
  report.preRangeBarCountDistribution = tsInWindow.reduce((a, s) => { const k = String(s.preRangeBarCount); a[k] = (a[k] ?? 0) + 1; return a; }, {} as Record<string, number>);

  const parity = tvAt.length === tsInWindow.length && tvOnly.length === 0 && tsOnly.length === 0 && exactMatches.length === tvAt.length;

  if (!parity) {
    // For TV-only origins, re-evaluate directly to show WHY TS did not signal (esp. range-bar count).
    ...
    report.firstMismatchDiagnostics = [...new Set([...tvOnly, ...tsOnly])].sort().slice(0, 10).map((origin) => {
      const i = idxByTime.get(origin);
      const c = candleByTime.get(origin);
      const ev = i !== undefined ? evaluateEurjpy01To05RangeBreakV1(candles.slice(0, i + 1)) : null;
      const p = i !== undefined && i > 0 ? candles[i - 1] : undefined;
      return { timestamp: origin, tvExpected: tvSet.has(origin), tsResult: tsSet.has(origin), open: c?.open ?? null, high: c?.high ?? null, low: c?.low ?? null, close: c?.close ?? null, previousHigh: p?.high ?? null, ema20: ev?.ema20 ?? null, ema20Back: ev?.ema20Back ?? null, atr14: ev?.atr14 ?? null, preHigh: ev?.preHigh ?? null, preLow: ev?.preLow ?? null, preRangeBarCount: ev?.preRangeBarCount ?? null, rangeReady: ev?.rangeReady ?? null, emaSlopeUp: ev?.emaSlopeUp ?? null, previousHighBreak: ev?.previousHighBreak ?? null, preRangeHighBreak: ev?.preRangeHighBreak ?? null, strategySignalQualified: ev?.strategySignalQualified ?? null };
    });
    report.firstDifferingCondition = tvOnly.length ? "TradingView entry absent from TypeScript signals (check preRangeBarCount)." : "TypeScript signal absent from TradingView export.";
  }

  report.verdict = parity ? "EURJPY_PINE_PARITY_CONFIRMED" : "EURJPY_PINE_PARITY_MISMATCH";
