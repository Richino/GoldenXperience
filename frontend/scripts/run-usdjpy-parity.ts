/**
 * USDJPY Pine-parity historical replay (read-only OANDA Practice MIDPOINT).
 * Replays the CURRENT TypeScript evaluator (`evaluateUsdjpyStrategyTrace`, V6
 * long-only) and compares to the canonical TradingView entries (long+short).
 * No orders, no execution gates, no modification of canonical Pine/CSV.
 */
...
const CACHE_PATH = "../research/frozen-strategies/USDJPY/oanda-h1-mid.csv";
const TV_CSV_PATH = "../research/frozen-strategies/USDJPY/tradingview-trades.csv";
...
function dataQuality(candles: readonly Candle[]) {
  ...
  return { first: candles[0]?.time ?? null, last: candles.at(-1)?.time ?? null, total: candles.length, incomplete, duplicates, outOfOrder, unexpectedGaps: gaps.length };
}
...
function parseTradingViewEntries(): TvEntry[] { const rows = readFileSync(TV_CSV_PATH, "utf8").replace(/^﻿/, "").trim().split(/\r?\n/); const h = rows[0]!.split(","); const tC = h.indexOf("Type"), dC = h.indexOf("Date and time"); const out: TvEntry[] = []; for (const row of rows.slice(1)) { const cols = row.split(","); const type = (cols[tC] ?? "").trim().toLowerCase(); if (!type.startsWith("entry")) continue; out.push({ wall: (cols[dC] ?? "").trim(), originUtc: nyWallToUtcISO((cols[dC] ?? "").trim()), direction: type.includes("short") ? "short" : "long" }); } return out; }

async function main() {
  const report: Record<string, unknown> = {};
  const token = environmentValue("OANDA_API_KEY") ?? environmentValue("OANDA_API_TOKEN");
  const tv = parseTradingViewEntries();
  const tvByOrigin = new Map(tv.map((e) => [e.originUtc, e.direction]));
  report.TRADINGVIEW_EXPORT_TIMEZONE_STATUS = "INFERRED_AMERICA_NEW_YORK";
  report.tradingViewEntries = tv.length;
  report.tvDirectionSplit = tv.reduce((a, e) => { a[e.direction] = (a[e.direction] ?? 0) + 1; return a; }, {} as Record<string, number>);
  report.tvOriginHourDistribution = tv.reduce((a, e) => { const k = e.originUtc.slice(11, 16); a[k] = (a[k] ?? 0) + 1; return a; }, {} as Record<string, number>);
  report.validOrigins_11to14 = tv.filter((e) => { const h = new Date(e.originUtc).getUTCHours(); return h >= 11 && h <= 14; }).length;
  report.invalidOriginTimes = tv.filter((e) => { const h = new Date(e.originUtc).getUTCHours(); return !(h >= 11 && h <= 14); }).length;

  if (!token) { report.OANDA_H1_DATA_FETCHED = "NO"; report.verdict = "USDJPY_PARITY_BLOCKED_NO_MARKET_DATA"; console.log(JSON.stringify(report, null, 2)); process.exitCode = 1; return; }
  const candles = await fetchMidpointH1(token);
  if (!candles.length) { report.OANDA_H1_DATA_FETCHED = "NO"; report.verdict = "USDJPY_PARITY_BLOCKED_NO_MARKET_DATA"; console.log(JSON.stringify(report, null, 2)); process.exitCode = 1; return; }
  writeCache(candles);
  ...
  // Replay CURRENT V6 TS evaluator (long-only) unmodified.
  const traced = evaluateUsdjpyStrategyTrace(candles);
  const tsSignals = traced.rows.filter((r) => r.finalLongSignal).map((r) => ({ ts: r.timestamp, direction: "long" as const }));
  const tvOrigins = [...tvByOrigin.keys()].sort();
  const minTv = Date.parse(tvOrigins[0]!), maxTv = Date.parse(tvOrigins.at(-1)!);
  const tsInWindow = tsSignals.filter((s) => Date.parse(s.ts) >= minTv && Date.parse(s.ts) <= maxTv);
  const tsByOrigin = new Map(tsInWindow.map((s) => [s.ts, s.direction]));

  const exactMatches = [...tsByOrigin.keys()].filter((t) => tvByOrigin.get(t) === tsByOrigin.get(t));
  const tvOnly = [...tvByOrigin.keys()].filter((t) => tsByOrigin.get(t) === undefined);
  const tsOnly = [...tsByOrigin.keys()].filter((t) => tvByOrigin.get(t) === undefined);
  const directionMismatches = [...tvByOrigin.keys()].filter((t) => tsByOrigin.has(t) && tvByOrigin.get(t) !== tsByOrigin.get(t));

  report.typescriptSignalsInWindow = tsInWindow.length;
  report.typescriptSignalsAllHistory = tsSignals.length;
  report.typescriptSignalOriginHourDistribution = tsInWindow.reduce((a, s) => { const k = s.ts.slice(11, 16); a[k] = (a[k] ?? 0) + 1; return a; }, {} as Record<string, number>);
  report.exactMatches = exactMatches.length;
  report.tvOnly = tvOnly.length;
  report.tvOnlyShorts = tvOnly.filter((t) => tvByOrigin.get(t) === "short").length;
  report.tvOnlyLongs = tvOnly.filter((t) => tvByOrigin.get(t) === "long").length;
  report.tsOnly = tsOnly.length;
  report.tsOnlySample = tsOnly.slice(0, 8);
  report.directionMismatches = directionMismatches.length;

  const parity = tvOnly.length === 0 && tsOnly.length === 0 && directionMismatches.length === 0 && exactMatches.length === tv.length;
  report.verdict = parity ? "USDJPY_PINE_PARITY_CONFIRMED" : "USDJPY_PINE_PARITY_MISMATCH";
