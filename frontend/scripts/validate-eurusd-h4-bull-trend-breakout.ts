...
const OUT = resolve(process.cwd(), "../api-server/research-v2/eurusd-h4-bull-trend-breakout-v1-validation");
...
const TV = {
  ...
  medianHoldHours: 28,
  year: {
    2023: { n: 21, winRate: 0.333, profitFactor: 0.80 },
    2024: { n: 11, winRate: 0.364, profitFactor: 1.67 },
    2025: { n: 23, winRate: 0.435, profitFactor: 1.75 },
    2026: { n: 6, winRate: 0.50, profitFactor: 3.75 },
  },
} as const;
...
function parityAssessment(mid: Metrics, midYears: Record<string, Metrics>) {
  ...
  const wrClose = mid.winRate != null && Math.abs(mid.winRate - TV.winRate) <= 0.06;
  const pfClose = mid.profitFactor != null && Math.abs(mid.profitFactor - TV.profitFactor) <= 0.25;
  const yearCounts = ([2023, 2024, 2025, 2026] as const).every((year) => Math.abs(midYears[year]!.n - TV.year[year].n) <= 5);
  const holdClose = mid.averageHoldingHours != null && Math.abs(mid.averageHoldingHours - TV.averageHoldHours) <= 20;
  const passed = countClose && wrClose && yearCounts;
  return {
    passed,
    countClose, wrClose, pfClose, yearCounts, holdClose,
    note: passed
      ? "OANDA H4 midpoint N/WR/year counts are close enough to the TradingView 2023-2026 headline to continue. No same-bar re-entry after an exit (TradingView no-pyramiding occupancy)."
      : "Material midpoint mismatch versus the TradingView headline. Executable cost conclusions withheld.",
    expected: TV,
  };
}
...
function classify(mid: Metrics, exec: Metrics, execYears: Record<string, Metrics>): Verdict {
  ...
  const materialPositiveYears = [2023, 2024, 2025].filter((year) => execYears[year]!.n >= 8 && (execYears[year]!.expectancyR ?? 0) > 0).length;
  if (exec.profitFactor > 1.05 && exec.expectancyR > 0.03 && materialPositiveYears >= 2) return "SURVIVES";
  return "MARGINAL";
}
...
async function main() {
  ...
  const first20 = full.closed.slice(0, 20).map((row) => ({
    ...
    rangeHigh: row.signal.rangeHigh,
  }));
  const protocol = {
    strategyId: STRATEGY_ID, name: STRATEGY_NAME, version: STRATEGY_VERSION, symbol: SYMBOL, timeframe: TIMEFRAME,
    frozen: true, longOnly: true, productionStrategyModified: false, deployed: false, ordersPlaced: false,
    rules: CONFIG,
    dailyDirection: "previous completed OANDA D candle only",
    range: "prior 12 completed H4 highs, current signal candle excluded",
    entry: "confirmed H4 midpoint close",
    executionOfficial: "stop-first; adverse stop gaps fill at the worse bid open; target gaps get no improvement; time exit at bar 30 close",
    executable: "long ask in / bid out; 1.5 ATR / 3.0 ATR rebuilt from frozen midpoint ATR around the executable entry; max hold 30 H4 bars",
  ...
  if (!parity.passed) {
    ...
    await writeFile(resolve(OUT, "RESULTS.json"), `${JSON.stringify({ generatedAt: new Date().toISOString(), protocol, parity, midpoint: mid, midpointYears: midYears, first20, verdict: "PARITY_FAILED" }, null, 2)}\n`);
    await writeFile(resolve(OUT, "FINAL_REPORT.md"), [
      "# EUR/USD H4 Bull Trend Breakout V1 — VALIDATION STOPPED",
      "",
      "Midpoint parity versus the supplied TradingView 2023-2026 headline failed materially. Executable cost conclusions are withheld. Frozen rules were not changed.",
      "",
      `| Window | N | WR | PF | EXP R | Avg hold h |`,
      `|---|---:|---:|---:|---:|---:|`,
      `| TV headline | ${TV.n} | ${pct(TV.winRate)} | ${fmt(TV.profitFactor)} | n/a | ${TV.averageHoldHours} |`,
      `| OANDA midpoint | ${mid.n} | ${pct(mid.winRate)} | ${fmt(mid.profitFactor)} | ${fmt(mid.expectancyR)} | ${fmt(mid.averageHoldingHours)} |`,
      "",
      [2023, 2024, 2025, 2026].map((year) => `- ${year}: OANDA N ${midYears[year]!.n} WR ${pct(midYears[year]!.winRate)} PF ${fmt(midYears[year]!.profitFactor)} vs TV N ${TV.year[year].n} WR ${pct(TV.year[year].winRate)} PF ${fmt(TV.year[year].profitFactor)}`).join("\n"),
      "",
      "First 20 OANDA entries:",
      "",
      ...first20.map((row, index) => `${index + 1}. ${row.signalTimestamp} long @ ${row.midEntry.toFixed(5)}`),
      "",
      "NO STRATEGY RULES WERE CHANGED. NO ORDERS WERE PLACED.",
      "",
    ].join("\n"));
    console.log(JSON.stringify({ verdict: "PARITY_FAILED", parity, midpoint: mid, midpointYears: midYears, outputDirectory: OUT }, null, 2));
    return;
  }
  ...
  const trades = full.closed.map((row) => ({
    ...
    rangeHigh: row.signal.rangeHigh, midStop: row.signal.midStop, midTarget: row.signal.midTarget,
  ...
  const artifact = {
    ...
    midpoint: mid, midpointYears: midYears, executable: exec, executableYears: execYears,
    financing: {
  ...
  const report = `# EUR/USD H4 Bull Trend Breakout V1 — FULL EXECUTABLE VALIDATION

Strategy ID: \`${STRATEGY_ID}\`. Frozen long-only rules. OANDA practice MBA. No production strategy was modified. No orders were placed.
...
|---|---:|---:|---:|---:|---:|---:|---:|
${[2023, 2024, 2025, 2026].map((year) => `| ${year} | ${TV.year[year].n} | ${pct(TV.year[year].winRate)} | ${fmt(TV.year[year].profitFactor)} | ${midYears[year]!.n} | ${pct(midYears[year]!.winRate)} | ${fmt(midYears[year]!.profitFactor)} | ${fmt(midYears[year]!.expectancyR)} |`).join("\n")}

Parity gate: **passed**. ${parity.note}

First 20 OANDA entries:

${first20.map((row, index) => `${index + 1}. ${row.signalTimestamp} long @ ${row.midEntry.toFixed(5)} ATR ${row.atr.toFixed(5)} rangeHigh ${row.rangeHigh.toFixed(5)}`).join("\n")}
...
- Current OANDA ${SYMBOL} longRate snapshot: ${currentRate ? `${(currentRate.longRate * 100).toFixed(4)}% annual (${currentRate.source})` : "unavailable"}
...
Same midpoint signals. Long enter ASK / stop-target-exit BID. ATR frozen. SL=1.5 ATR, TP=3.0 ATR around executable entry. Max hold 30 H4 bars. Stop-first. Adverse gaps fill at the worse bid open.
...
- bid/ask cost drag: ${fmt(bidAskDrag)}R/trade
...
- 2023 remains negative midpoint? ${ (midYears[2023]!.expectancyR ?? 0) < 0 ? "YES" : "NO" } (EXP ${fmt(midYears[2023]!.expectancyR)}R). After bid/ask: ${ (execYears[2023]!.expectancyR ?? 0) < 0 ? "still negative" : "turned positive" } (EXP ${fmt(execYears[2023]!.expectancyR)}R).
- 2024 after bid/ask: ${fmt(execYears[2024]!.expectancyR)}R, PF ${fmt(execYears[2024]!.profitFactor)}.
- 2025 after bid/ask: ${fmt(execYears[2025]!.expectancyR)}R, PF ${fmt(execYears[2025]!.profitFactor)}.
...
- 2023 is kept in the headline result.
...
`;
  ...
  console.log(JSON.stringify({
    verdict, midpoint: mid, executable: exec, financing: artifact.financing, costs: artifact.costs,
    stability: artifact.stability, parity: { passed: parity.passed, n: mid.n }, outputDirectory: OUT,
  }, null, 2));
}
