    ...
    <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/60 p-4" data-pull-to-refresh-ignore="true" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className="max-h-[90dvh] w-full max-w-md overflow-y-auto rounded-3xl bg-white p-5 text-zinc-900 shadow-2xl dark:bg-zinc-950 dark:text-zinc-100" role="dialog" aria-modal="true" aria-labelledby="trend-pullback-title">
        <header className="flex items-start justify-between gap-4">
          <div><p className="text-xs font-semibold uppercase tracking-wide text-emerald-600">TrendPullbackV1 · loose · M15</p><h2 id="trend-pullback-title" className="mt-1 text-xl font-semibold">{validPlan ? status : "No trade plan"}</h2></div>
        ...
        {validPlan ? <>
          ...
          <ul className="mt-3 space-y-1 text-sm text-zinc-600 dark:text-zinc-300">{result.reasons.map((reason) => <li key={reason}>{reason}</li>)}</ul>
          {result.warnings.length ? <ul className="mt-3 space-y-1 rounded-xl border border-amber-200 p-3 text-sm text-amber-700 dark:border-amber-900 dark:text-amber-400">{result.warnings.map((warning) => <li key={warning}>⚠ {warning}</li>)}</ul> : null}
