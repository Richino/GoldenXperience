  ...
  return (
    <div className="dashboard-view space-y-5">
      ...
      <section className="app-card overflow-hidden">
        ...
        {overview.trades.length ? <div>{overview.trades.slice(0, 6).map((trade) => <Link key={trade.id} href={`/signals?instrument=${trade.instrument}&trade=${trade.id}`} className="dashboard-row pressable flex items-center justify-between gap-3 px-5 py-3.5 md:px-6"><div><p className="text-sm font-medium">#{trade.tradeSequence} · {displayNameFor(trade.instrument)} <span className={trade.direction === "long" ? "text-[color:var(--success)]" : "text-[color:var(--danger)]"}>{trade.direction}</span></p><p className="mt-0.5 text-xs text-[color:var(--muted)]">{time(trade.openedAt)} · {trade.status}</p></div><p className="metric-number text-sm font-semibold">{trade.resultR === null ? "Open" : `${trade.resultR.toFixed(2)}R`}</p></Link>)}</div> : <div className="border-t border-[color:var(--border)] px-5 py-7 md:px-6"><p className="empty-state"><span className="empty-state-dot" />No automatic paper trades yet. The first valid completed-M15 setup will start Batch 1.</p></div>}
      </section>
