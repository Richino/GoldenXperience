...
type DiagnosticTrade = { id: string; instrument: string; decisionTime: string; direction: "long" | "short"; entry: number; stop: number; target: number; plannedR: number; spreadPips: number; session: string; outcome: "target_first" | "stop_first" | "forced_close" | "unresolved" | "ambiguous" | null; resultR: number | null; mfeR: number | null; maeR: number | null; executionStatus: "pending" | "accepted" | "overlapping"; blockedByCandidateId: string | null; simulatedEntryAt: string | null; simulatedExitAt: string | null; notEvaluated: string[] };
...
function ResearchSectionHead({
  ...
  return (
    <div className="research-section-head">
      ...
      {description ? (
        <p className={`mt-0.5 text-xs text-[color:var(--muted)] ${descriptionClassName}`.trim()}>{description}</p>
      ) : null}
    </div>
  );
}
...
export function ResearchView() {
  ...
  const outcomeLabel = (outcome: DiagnosticTrade["outcome"]) => outcome === "target_first" ? "Target first" : outcome === "stop_first" ? "Stop first" : outcome === "forced_close" ? "Forced close" : outcome === "ambiguous" ? "Ambiguous" : outcome === "unresolved" ? "Unresolved" : "Not labeled";
  ...
  return (
    <div className="research-view research-minimal space-y-8 lg:space-y-10">
      <header>
        <h1 className="text-display">Research</h1>
        <p className="mt-1 text-sm text-[color:var(--muted)]">Backtests and paper cycle</p>
      </header>

      <ForwardView />

      <section className="research-minimal-section" aria-label="Run research">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <ResearchSectionHead title="Run research" />
      ...
      <section className="research-minimal-section" aria-label="Baseline">
        <ResearchSectionHead title={`Baseline · ${selectedInstrument?.displayName ?? pair.replace("_", "/")}`} />
        <div className="research-metric-grid mt-4">{metrics.map(([label, metric]) => <div key={label} className="research-metric-cell"><p className="research-stat-label">{label}</p><p className="research-stat-value metric-number">{metric}</p></div>)}</div>
        <p className="research-footnote">Raw {value.candidates} · Executable {value.executable_candidates} · Overlapping {value.overlapping_candidates} · Pending {value.pending_candidates} · Target {value.target_first} · Stop {value.stop_first} · Forced exits {value.forced_close} · Unresolved {value.unresolved} · Ambiguous {value.ambiguous}</p>
      </section>
      <section className="research-minimal-section" aria-label="Raw versus executable">
        <ResearchSectionHead title="Raw vs executable" />
      ...
      <section className="research-minimal-section" aria-label="Resolved versus conservative">
        <ResearchSectionHead title="Resolved vs conservative" />
      ...
      <section className="research-minimal-section hidden" aria-hidden="true">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <ResearchSectionHead title="Retired experiment" />
          <div className="research-toolbar">
      ...
      {experiment && <section className="research-minimal-section">
        ...
        {holdout ? <div className="research-panel mt-5"><p className="research-panel-title">{holdout.instrument.replace("_", "/")} · {holdout.direction === "all" ? "All directions" : `${holdout.direction} only`} </p><p className="research-panel-copy">{new Date(holdout.range_start).toISOString().slice(0, 10)} to {new Date(holdout.range_end).toISOString().slice(0, 10)} · {holdout.sessions.join(" + ")}</p>{holdoutWorking && <><div className="mt-4 h-2 overflow-hidden rounded-full bg-[color:var(--border)]"><div className="h-full rounded-full bg-[color:var(--accent)] transition-[width] duration-500" style={{ width: `${holdoutRun?.details.progressPercent ?? 0}%` }} /></div><p className="mt-2 text-xs text-[color:var(--muted)]">{holdoutRun?.details.phase ?? "Starting locked holdout"} · {holdoutRun?.details.progressPercent ?? 0}%</p></>}{holdout.status === "failed" && <p className="mt-4 text-sm text-[color:var(--danger)]">{holdout.error ?? holdoutRun?.error ?? "The locked holdout failed."}</p>}{holdout.summary && <><div className="research-compare-grid mt-5">{[["Resolved sample", holdout.summary.executable.sample_size, optionalSample(holdoutConservative)], ["Win rate", percent(holdout.summary.executable.win_rate), optionalPercent(holdoutConservative)], ["Average R", rValue(holdout.summary.executable.average_r), optionalR(holdoutConservative, "average_r")], ["Profit factor", holdout.summary.executable.profit_factor === null ? "—" : holdout.summary.executable.profit_factor.toFixed(2), optionalProfitFactor(holdoutConservative)], ["Drawdown", rValue(holdout.summary.executable.drawdown_r), optionalR(holdoutConservative, "drawdown_r")]].map(([label, metric, conservative]) => <div key={String(label)} className="research-compare-item"><p className="research-compare-label">{label}</p><ResearchInlineStat label="Resolved only" value={metric} /><ResearchInlineStat label="Conservative" value={conservative} /></div>)}</div>{!holdoutConservative && <p className="mt-4 text-xs text-[color:var(--muted)]">Conservative outcomes not recorded for this holdout.</p>}<p className="research-footnote !px-0">Raw {holdout.summary.raw_candidates} · Executable {holdout.summary.executable_candidates} · Overlapping {holdout.summary.overlapping_candidates} · Target {holdout.summary.target_first} · Stop {holdout.summary.stop_first} · Unresolved {holdout.summary.unresolved} · Ambiguous {holdout.summary.ambiguous}</p></>}</div> : <p className="mt-4 text-sm text-[color:var(--muted)]">No holdout yet.</p>}
      </section>}
      <section className="research-minimal-section">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <ResearchSectionHead
            title="Day-trading validation"
          />
          <ResearchPrimaryButton onClick={runWalkForward} disabled={runningWalkForward || working} loading={runningWalkForward}>
            {runningWalkForward ? "Running validation…" : "Run 4y / 1y validation"}
          </ResearchPrimaryButton>
        </div>
        {walkForward ? <div className="research-panel mt-5">
          <p className="research-panel-title">{walkForward.instrument.replace("_", "/")} · locked validation</p>
          <p className="research-panel-copy">4-year development · 1-year out-of-sample</p>
