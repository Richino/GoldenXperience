...
/** Same cutoff Signals uses for ChartOptionSheet vs the desktop strip. */
...
function getMobileLayoutServer() {
  return false;
}
...
export function SelectMenu<T extends string | number>({
  ...
  return (
    <div ref={rootRef} className={`relative ${fullWidth ? "w-full" : ""} ${className}`}>
      ...
      {isMobileLayout ? (
        <MobileSheet open={open} onClose={() => setOpen(false)} title={ariaLabel}>
          <div role="listbox" aria-label={ariaLabel} className="sheet-option-list">
            {options.map((option) => {
