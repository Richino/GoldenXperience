...
import type { EuraudStrategyFeatures, MarketRegime, StrategyCondition, StrategyEvaluationInput } from "@/lib/strategy/types";
import type { Candle } from "@/types/forex";

export const EURAUD_STRATEGY_ID = "euraud_strategy" as const;
export const EURAUD_STRATEGY_NAME = "EURAUD Bull Consensus V1" as const;
export const EURAUD_STRATEGY_VERSION = "V1" as const;
export const EURAUD_STRATEGY_CONFIG_VERSION = "euraud-v1-frozen" as const;
export const EURAUD_STRATEGY_SYMBOL = "EUR_AUD" as const;
export const EURAUD_STRATEGY_TIMEFRAME = "H1" as const;
/**
 * Frozen contract for "GX EURAUD Bull Consensus V1 - 1 to 2 RR".
 ...
 */
export const EURAUD_STRATEGY_CONFIG = Object.freeze({ symbol: EURAUD_STRATEGY_SYMBOL, timeframe: EURAUD_STRATEGY_TIMEFRAME, originHourUtc: 11, startTimeUtc: "2023-01-01T00:00:00.000Z", direction: "LONG_ONLY", emaFastPeriod: 20, emaSlowPeriod: 50, atrPeriod: 14, consensus: ">= +3", extraFilters: "NONE", stopAtr: 1, rewardR: 2, maxHoldBars: 3, executionEnabled: true, adaptiveParametersMutable: false });

export interface EuraudPineEvaluation {
...
export interface EuraudStrategyEvaluationOptions { timeframe?: string; hasActivePosition?: boolean; duplicateSignal?: boolean; }
...
const START = Date.parse(EURAUD_STRATEGY_CONFIG.startTimeUtc);
const ORIGIN_HOUR = EURAUD_STRATEGY_CONFIG.originHourUtc;
 ...
 * 11:00 UTC origin on/after the frozen start, indicators/history ready, ATR>0,
 * and four-vote consensus >= +3. No other filter exists.
 */
export function evaluateEuraudBullConsensusV1(input: readonly Candle[]): EuraudPineEvaluation {
  ...
  const origin = Boolean(x && Date.parse(x.time) >= START && exactH1(new Date(x.time)) && new Date(x.time).getUTCHours() === ORIGIN_HOUR);
...
function block(input: StrategyEvaluationInput, q: boolean, o: EuraudStrategyEvaluationOptions) {
  if (!q) return "NO_STRATEGY_SIGNAL";
  if (input.instrument !== EURAUD_STRATEGY_SYMBOL) return "WRONG_INSTRUMENT";
  if ((o.timeframe ?? "H1") !== "H1") return "WRONG_TIMEFRAME";
  if (o.duplicateSignal) return "DUPLICATE_SIGNAL";
  if (o.hasActivePosition) return "EURAUD_V1_POSITION_ACTIVE";
  if (input.dataSource !== "oanda") return "OANDA_CONNECTION_UNAVAILABLE";
  if (!finite(input.ask)) return "OANDA_PRICE_UNAVAILABLE";
  return null;
}

function regime(p: EuraudPineEvaluation, t: string): MarketRegime {
  return { regime: "mixed", trendDirection: p.ema20 === null || p.ema50 === null ? "none" : p.ema20 > p.ema50 ? "up" : p.ema20 < p.ema50 ? "down" : "none", trendStrength: 0, volatility: "normal", atr: p.atr14, atrPips: p.atr14 === null ? null : p.atr14 / pipSizeFor(EURAUD_STRATEGY_SYMBOL), momentumState: "steady", emaFast: p.ema20, emaMid: p.ema50, emaSlow: null, slopeAtrPerBar: null, rangeHigh: null, rangeLow: null, rangeWidthAtr: null, rangeAgeBars: null, lookbackBars: 50, evaluatedAt: t };
}

export function evaluateEuraudStrategy(input: StrategyEvaluationInput, options: EuraudStrategyEvaluationOptions = {}): StrategyCandidate<typeof EURAUD_STRATEGY_ID> {
  const p = evaluateEuraudBullConsensusV1(input.candles1h);
  const q = p.strategySignalQualified && input.instrument === EURAUD_STRATEGY_SYMBOL && (options.timeframe ?? "H1") === "H1";
  ...
  const features: EuraudStrategyFeatures = {
    strategyId: EURAUD_STRATEGY_ID, strategyName: EURAUD_STRATEGY_NAME, strategyVersion: EURAUD_STRATEGY_VERSION, symbol: EURAUD_STRATEGY_SYMBOL, timeframe: "H1",
    signalKey: q && p.originTime ? `EURAUD_1100_LONG:${p.originTime}` : null, originTime: p.originTime,
  ...
  const conditions = [
    condition("Pine EURAUD instrument", input.instrument === EURAUD_STRATEGY_SYMBOL, "EUR_AUD only.", input.instrument),
    condition("Pine H1", (options.timeframe ?? "H1") === "H1", "H1 only.", options.timeframe ?? "H1"),
    condition("Pine completed 11:00 UTC origin", Boolean(p.originTime && Date.parse(p.originTime) >= START && exactH1(new Date(p.originTime)) && new Date(p.originTime).getUTCHours() === ORIGIN_HOUR), "Completed 11:00 UTC candle on/after the frozen start date.", p.originTime ?? "unavailable"),
    ...
    condition("Execution EURAUD position", !options.hasActivePosition, "Pyramiding scoped to EURAUD V1.", options.hasActivePosition ? "active" : "clear", false),
  ...
  return {
    family: EURAUD_STRATEGY_ID, version: EURAUD_STRATEGY_VERSION, configVersion: EURAUD_STRATEGY_CONFIG_VERSION, regime: regime(p, evaluatedAt),
    qualifyReason: q ? executionAllowed ? "EURAUD Pine signal qualified and execution is allowed." : `EURAUD Pine signal qualified; execution blocked: ${executionBlockReason}.` : "EURAUD Pine signal did not qualify.",
    ...
    features: { trend15m: "mixed", trend1h: p.ema20 === null || p.ema50 === null ? "mixed" : p.ema20 > p.ema50 ? "bullish" : p.ema20 < p.ema50 ? "bearish" : "mixed", trend4h: null, ema21: null, ema50: p.ema50, ema200: null, rsi14: null, atr14: p.atr14, atrPips: p.atr14 === null ? null : p.atr14 / pipSizeFor(EURAUD_STRATEGY_SYMBOL), structureHighs: 0, structureLows: 0, evaluationMode: input.evaluationMode ?? "live", euraudStrategy: features },
    summary: q ? executionAllowed ? "EURAUD Bull Consensus V1 LONG signal." : `EURAUD Bull Consensus V1 execution blocked: ${executionBlockReason}.` : "EURAUD Bull Consensus V1 WAIT.",
...
export const resolveEuraudExit = (input: FrozenH1ExitInput) => resolveFrozenH1Exit(input);
