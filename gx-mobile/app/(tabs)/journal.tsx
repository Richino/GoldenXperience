...
function Summary({ label, value, tone }: { label: string; value: string; tone?: object }) {
  const styles = useThemedStyles(createStyles); return <View style={styles.summaryItem}><Text style={[styles.summaryValue, tone]}>{value}</Text><Text style={styles.summaryLabel}>{label}</Text></View>; }
...
export default function JournalScreen() {
  ...
  return <View style={styles.root}><ScrollView contentContainerStyle={[styles.content, { paddingTop: Math.max(insets.top + 12, 30), paddingBottom: DOCK_CLEARANCE + Math.max(insets.bottom, 8) }]} showsVerticalScrollIndicator={false} scrollEventThrottle={16} onScroll={(event) => { const next = event.nativeEvent.contentOffset.y > 52; setShowFloatingBell((current) => current === next ? current : next); }} keyboardShouldPersistTaps="handled" refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={colors.primary} />}>
    <View style={styles.header}><Text style={styles.title}>Trades</Text><Pressable onPress={() => setNotificationsOpen(true)} style={styles.headerBell} accessibilityRole="button" accessibilityLabel="Open notifications"><SymbolView name={{ ios: 'bell', android: 'notifications', web: 'notifications' }} size={21} tintColor={colors.textSecondary} /></Pressable></View>
    {loadedOnce ? <>
    ...
    <View style={styles.toolbar}>
      ...
      <View style={styles.search}><Search size={17} color={colors.textMuted} strokeWidth={2} /><TextInput value={query} onChangeText={setQuery} placeholder="Search trades..." placeholderTextColor={colors.textMuted} style={styles.searchInput} accessibilityLabel="Search trades" /></View>
  ...
  </ScrollView><DockFade height={96} />{showFloatingBell && !notificationsOpen ? <View style={[styles.floatingBellWrap, { top: insets.top + 8 }]}><Pressable onPress={() => setNotificationsOpen(true)} style={styles.floatingBell} accessibilityRole="button" accessibilityLabel="Open notifications"><SymbolView name={{ ios: 'bell', android: 'notifications', web: 'notifications' }} size={20} tintColor={colors.textPrimary} /></Pressable></View> : null}<NotificationDrawer visible={notificationsOpen} onClose={() => setNotificationsOpen(false)} /></View>;
}

const createStyles = (colors: ThemeColors) => StyleSheet.create({
  root: { flex: 1, backgroundColor: colors.background }, content: { paddingHorizontal: 16, gap: 14 }, header: { minHeight: 44, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, title: { fontSize: 28, fontFamily: theme.fonts.sansBold, color: colors.textPrimary, letterSpacing: -0.8 }, headerBell: { width: 44, height: 44, alignItems: 'center', justifyContent: 'center' }, floatingBellWrap: { position: 'absolute', right: 16, zIndex: 40 }, floatingBell: { width: 44, height: 44, borderRadius: 22, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.surface, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.cardBorder, ...shadows(colors).card },
  summaryCard: { paddingVertical: 16 }, summaryGrid: { flexDirection: 'row' }, summaryItem: { flex: 1, minWidth: 0, paddingHorizontal: 7 }, summaryValue: { fontSize: 16, fontFamily: theme.fonts.monoSemiBold, color: colors.textPrimary }, summaryLabel: { marginTop: 4, fontSize: 8, fontFamily: theme.fonts.sansSemiBold, letterSpacing: 0.65, color: colors.textMuted, textTransform: 'uppercase' },
  toolbar: { gap: 9 }, tabs: { minHeight: 45, flexDirection: 'row', padding: 4, borderRadius: 13, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.border, backgroundColor: colors.surface }, tab: { flex: 1, minHeight: 35, alignItems: 'center', justifyContent: 'center', borderRadius: 9 }, tabActive: { backgroundColor: colors.primary }, tabText: { fontSize: 12, fontFamily: theme.fonts.sansSemiBold, color: colors.textSecondary }, tabTextActive: { color: colors.background }, tabCount: { fontFamily: theme.fonts.monoMedium, opacity: 0.78 }, search: { minHeight: 44, flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 13, borderRadius: 12, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.border, backgroundColor: colors.surface }, searchInput: { flex: 1, color: colors.textPrimary, fontSize: 13, fontFamily: theme.fonts.sans }, filters: { flexDirection: 'row', gap: 7 }, filter: { flex: 1, minHeight: 38, alignItems: 'center', justifyContent: 'center', borderRadius: 10, backgroundColor: colors.surfaceInset }, filterActive: { backgroundColor: colors.primarySoft }, filterText: { fontSize: 12, fontFamily: theme.fonts.sansMedium, color: colors.textSecondary }, filterTextActive: { color: colors.primary },
  cards: { gap: 9 }, tradeCard: { position: 'relative', overflow: 'hidden', gap: 10, paddingVertical: 14, paddingHorizontal: 16, paddingLeft: 18, borderRadius: 16, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.cardBorder, backgroundColor: colors.surface, ...shadows(colors).card }, lightTradeShadow: lightCardShadow, pressed: { opacity: 0.8 }, tradeAccent: { position: 'absolute', left: 0, top: 0, bottom: 0, width: 3 }, accentPositive: { backgroundColor: colors.primary }, accentNegative: { backgroundColor: colors.danger }, tradeTop: { flexDirection: 'row', alignItems: 'baseline', justifyContent: 'space-between', gap: 10 }, pair: { fontSize: 16, fontFamily: theme.fonts.sansSemiBold, color: colors.textPrimary }, tradeR: { fontSize: 15, fontFamily: theme.fonts.monoSemiBold }, tradeSub: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', minHeight: 17 }, side: { fontSize: 10, fontFamily: theme.fonts.sansBold, letterSpacing: 0.55 }, meta: { fontSize: 11, fontFamily: theme.fonts.sansMedium, color: colors.textMuted }, resultBadge: { paddingHorizontal: 7, paddingVertical: 3, borderRadius: 6, fontSize: 9, fontFamily: theme.fonts.sansBold, letterSpacing: 0.65, overflow: 'hidden' }, levels: { flexDirection: 'row', gap: 5 }, level: { flex: 1, minWidth: 0 }, levelLabel: { fontSize: 8, fontFamily: theme.fonts.sansSemiBold, letterSpacing: 0.55, textTransform: 'uppercase', color: colors.textMuted }, levelValue: { marginTop: 3, fontSize: 11, fontFamily: theme.fonts.monoMedium, color: colors.textPrimary }, flow: { fontSize: 13, fontFamily: theme.fonts.monoMedium, color: colors.textPrimary }, tradeFoot: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 10 }, footText: { flex: 1, fontSize: 11, fontFamily: theme.fonts.sans, color: colors.textMuted }, tradeMoney: { fontSize: 12, fontFamily: theme.fonts.monoSemiBold }, positive: { color: colors.primary }, negative: { color: colors.danger }, muted: { color: colors.textMuted }, error: { fontSize: 12, fontFamily: theme.fonts.sansMedium, color: colors.danger }, empty: { paddingVertical: 30, textAlign: 'center', fontSize: 13, fontFamily: theme.fonts.sans, color: colors.textMuted }, emptyCard: { paddingVertical: 24, alignItems: 'center' }, emptyTitle: { fontSize: 15, fontFamily: theme.fonts.sansSemiBold, color: colors.textPrimary }, emptyDetail: { maxWidth: 250, marginTop: 5, textAlign: 'center', fontSize: 12, lineHeight: 17, fontFamily: theme.fonts.sans, color: colors.textSecondary }, loading: { minHeight: 260, alignItems: 'center', justifyContent: 'center' }, loadMore: { minHeight: 44, alignItems: 'center', justifyContent: 'center', borderRadius: 12, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.border, backgroundColor: colors.surface }, loadMoreText: { fontSize: 13, fontFamily: theme.fonts.sansSemiBold, color: colors.textSecondary },
});
