...
export default function SettingsScreen() {
  ...
  return <View style={styles.root}><ScrollView contentContainerStyle={[styles.content, { paddingTop: Math.max(insets.top + 10, 28), paddingBottom: 118 + Math.max(insets.bottom, 8) }]} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" refreshControl={<RefreshControl tintColor={colors.primary} refreshing={loading} onRefresh={() => void load()} />}>
    ...
    <HomeCard style={styles.card} accessibilityLabel="Paper trading risk settings"><View style={styles.sectionHead}><View><Text style={styles.section}>Paper trading</Text><Text style={styles.helper}>Practice-entry risk limits</Text></View><Text style={[styles.status, paused ? styles.paused : styles.active]}>{paused ? 'Paused' : 'Accepting'}</Text></View>
      {loading && !policy ? <ActivityIndicator style={styles.loader} color={colors.primary} /> : <>
        ...
        <View style={styles.switchRow}><View style={styles.switchCopy}><Text style={styles.switchTitle}>Allow new entries</Text></View><Switch value={!paused} onValueChange={(value) => setPaused(!value)} trackColor={{ false: colors.surfaceRaised, true: colors.primarySoft }} thumbColor={!paused ? colors.primary : colors.textSecondary} /></View>
...
function Field({ value, onChangeText, label, placeholder }: { value: string; onChangeText: (value: string) => void; label: string; placeholder: string }) {
  const colors = useThemeColors();
  const styles = useThemedStyles(createStyles);
  return <View style={styles.fieldWrap}><TextInput value={value} onChangeText={onChangeText} keyboardType="decimal-pad" placeholder={placeholder} placeholderTextColor={colors.textMuted} style={styles.input} accessibilityLabel={label} /><Text style={styles.suffix}>%</Text></View>;
}
function Row({ label, value, last = false, onPress }: { label: string; value: string; last?: boolean; onPress?: () => void }) {
  const colors = useThemeColors();
  const styles = useThemedStyles(createStyles); return <Pressable disabled={!onPress} onPress={onPress} style={[styles.row, last ? styles.last : null]} accessibilityRole={onPress ? 'button' : undefined}><Text style={styles.rowLabel}>{label}</Text><View style={styles.rowEnd}><Text style={styles.rowValue}>{value}</Text>{onPress ? <SymbolView name={{ ios: 'chevron.right', android: 'chevron_right', web: 'chevron_right' }} size={14} tintColor={colors.textMuted} /> : null}</View></Pressable>; }
      ...
      {options.map((option, index) => (
        ...
        >
          ...
          {selected === option.value ? (
            <SymbolView name={{ ios: 'checkmark', android: 'check', web: 'check' }} size={18} tintColor={colors.primary} />
          ) : null}
        </Pressable>
      ))}
    </BottomDrawer>
...
const createStyles = (colors: ThemeColors) => StyleSheet.create({
  topControls: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }, iconButton: { width: 30, height: 30, alignItems: 'center', justifyContent: 'center' }, referenceCard: { paddingTop: 27, paddingBottom: 27 }, referenceHeading: { fontSize: 16, fontFamily: theme.fonts.sansSemiBold, color: colors.textPrimary }, segmentRow: { minHeight: 80, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, segmentLabel: { fontSize: 16, fontFamily: theme.fonts.sans, color: colors.textSecondary }, segment: { width: 178 }, segmentTrack: { flexDirection: 'row', padding: 4, borderRadius: 17, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.border, backgroundColor: colors.surfaceInset }, segmentItem: { flex: 1, minHeight: 42, alignItems: 'center', justifyContent: 'center', borderRadius: 13 }, segmentItemOn: { borderWidth: StyleSheet.hairlineWidth, borderColor: colors.border, backgroundColor: colors.surfaceRaised }, segmentText: { fontSize: 15, fontFamily: theme.fonts.sansMedium, color: colors.textSecondary }, segmentTextOn: { color: colors.textPrimary }, controlLabel: { marginTop: 26, fontSize: 16, fontFamily: theme.fonts.sans, color: colors.textSecondary }, soundSelect: { marginTop: 14, minHeight: 60, paddingHorizontal: 16, borderRadius: 14, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.border, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, soundSelectText: { fontSize: 16, fontFamily: theme.fonts.sans, color: colors.textPrimary }, preview: { marginTop: 16, minHeight: 60, borderRadius: 14, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.border, alignItems: 'center', justifyContent: 'center' }, previewText: { fontSize: 16, fontFamily: theme.fonts.sansSemiBold, color: colors.textPrimary }, volumeRow: { marginTop: 16, flexDirection: 'row', alignItems: 'center', gap: 14 }, volumeTrack: { flex: 1, height: 10, borderRadius: 5, backgroundColor: colors.surfaceRaised }, volumeFill: { width: '50%', height: '100%', borderRadius: 5, backgroundColor: colors.primary }, volumeKnob: { position: 'absolute', left: '50%', top: -6, width: 22, height: 22, marginLeft: -11, borderRadius: 11, backgroundColor: colors.textPrimary }, volumeLabel: { width: 50, textAlign: 'right', fontSize: 14, fontFamily: theme.fonts.sansSemiBold, color: colors.textPrimary }, noticeRow: { minHeight: 70, marginTop: 18, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, noticeLast: { marginTop: 4 }, noticeTitle: { fontSize: 16, fontFamily: theme.fonts.sansMedium, color: colors.textPrimary }, noticeDetail: { marginTop: 3, fontSize: 13, fontFamily: theme.fonts.sans, color: colors.textSecondary }, outlineButton: { minHeight: 58, minWidth: 76, paddingHorizontal: 14, borderRadius: 14, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.border, alignItems: 'center', justifyContent: 'center' }, outlineText: { fontSize: 16, fontFamily: theme.fonts.sansMedium, color: colors.textPrimary },
  root: { flex: 1, backgroundColor: colors.background }, content: { paddingHorizontal: 16, gap: 14 }, title: { fontSize: 28, fontFamily: theme.fonts.sansBold, color: colors.textPrimary, letterSpacing: -0.7 }, subtitle: { marginTop: 3, fontSize: 13, fontFamily: theme.fonts.sans, color: colors.textSecondary }, card: { paddingVertical: 18 }, section: { fontSize: 10, fontFamily: theme.fonts.sansMedium, letterSpacing: 1.4, color: colors.textMuted, textTransform: 'uppercase' }, row: { minHeight: 52, paddingVertical: 13, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: colors.border }, last: { borderBottomWidth: 0, paddingBottom: 0 }, rowLabel: { flex: 1, fontSize: 14, fontFamily: theme.fonts.sansMedium, color: colors.textPrimary }, rowEnd: { maxWidth: '58%', flexDirection: 'row', alignItems: 'center', gap: 5 }, rowValue: { flexShrink: 1, fontSize: 12, fontFamily: theme.fonts.sans, color: colors.textSecondary, textAlign: 'right' }, sectionHead: { flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between' }, helper: { marginTop: 4, fontSize: 11.5, lineHeight: 16, fontFamily: theme.fonts.sans, color: colors.textSecondary }, status: { fontSize: 11, fontFamily: theme.fonts.sansSemiBold }, active: { color: colors.primary }, paused: { color: colors.danger }, loader: { marginVertical: 36 }, field: { marginTop: 18, marginBottom: 7, fontSize: 11, fontFamily: theme.fonts.sansMedium, color: colors.textSecondary }, optional: { color: colors.textMuted }, fieldWrap: { height: 46, flexDirection: 'row', alignItems: 'center', borderRadius: 12, backgroundColor: colors.surfaceInset, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.border, paddingHorizontal: 13 }, input: { flex: 1, height: '100%', fontSize: 15, fontFamily: theme.fonts.monoMedium, color: colors.textPrimary }, suffix: { fontSize: 13, fontFamily: theme.fonts.sansMedium, color: colors.textMuted }, grid: { flexDirection: 'row', flexWrap: 'wrap', gap: 7 }, choice: { width: 40, height: 40, borderRadius: 10, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.surfaceInset, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.border }, choiceOn: { backgroundColor: colors.primarySoft, borderColor: colors.primaryBorder }, choiceText: { fontSize: 13, fontFamily: theme.fonts.monoMedium, color: colors.textSecondary }, choiceTextOn: { color: colors.primary }, switchRow: { marginTop: 20, paddingTop: 16, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: colors.border, flexDirection: 'row', alignItems: 'center', gap: 14 }, switchCopy: { flex: 1 }, switchTitle: { fontSize: 14, fontFamily: theme.fonts.sansMedium, color: colors.textPrimary },   error: { marginTop: 12, fontSize: 12, fontFamily: theme.fonts.sansMedium, color: colors.danger }, signOut: { marginTop: 14, minHeight: 44, borderRadius: 12, backgroundColor: colors.dangerSoft, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 }, signOutText: { fontSize: 13, fontFamily: theme.fonts.sansSemiBold, color: colors.danger },
  drawerRow: { minHeight: 56, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 12, paddingVertical: 10, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: colors.border, borderRadius: 10 },
  drawerRowActive: { backgroundColor: colors.primaryMuted },
  drawerRowLast: { borderBottomWidth: 0 },
  drawerCopy: { flex: 1, minWidth: 0, paddingRight: 10 },
  drawerLabel: { fontSize: 14, fontFamily: theme.fonts.sansSemiBold, color: colors.textPrimary },
  drawerDetail: { marginTop: 2, fontSize: 11.5, lineHeight: 16, fontFamily: theme.fonts.sans, color: colors.textSecondary },
});
